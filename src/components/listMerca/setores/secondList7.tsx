...
export const SecondList7 = () => {
  ...
  const { list7, handleUpdateNameSet, setChaveKey, setModalReturn, setModalList, nameSetor, toogleItem, setQtdListReturn } = useList();
  ...
  return (
    <>
      {secondMinimized &&
        <div className={`w-full h-auto p-2 px-4 rounded-full flex justify-between items-center ${list7.length > 0 ? 'border-b border-b-blue-500' : 'border-b'} bg-background text-primary-foreground`}>
          <h1 className="">{nameSetor[chaveShowEdit]}</h1>
          <div className="flex items-center gap-2">
            <div className="">
              {list7.length} {list7.length !== 0 && list7.length !== 1 ? 'itens' : 'item'}
            </div>
