...
export const SecondList8 = () => {
  ...
  const chaveShowEdit = 8;
  const [showInput, setShowInput] = useState<boolean>(false);
  const [secondMinimized, setSecondMinimized] = useState<boolean>(false);

  const { list8, handleUpdateNameSet, setChaveKey, setModalReturn, setModalList, nameSetor, toogleItem, setQtdListReturn } = useList();
  ...
  return (
    <>
      {secondMinimized &&
        <div className={`w-full h-auto p-2 px-4 rounded-full flex justify-between items-center ${list8.length > 0 ? 'border-b border-b-blue-500' : 'border-b'} bg-background text-primary-foreground`}>
          <h1 className="">{nameSetor[chaveShowEdit]}</h1>
          <div className="flex items-center gap-2">
            <div className="">
              {list8.length} {list8.length !== 0 && list8.length !== 1 ? 'itens': 'item'}
            </div>
      ...
      {!secondMinimized &&
        ...
        <div className={`w-full h-auto flex flex-col justify-between ${list8.length > 0 ? 'border border-blue-500' : 'border'} p-4 rounded-3xl bg-input`}>
          <h1>
